...
///
/// Supports three signing modes, which differ in whether Fordefi broadcasts the
/// transaction and in which entry point is available:
...
/// - **Native auto** (`chain` = `Some(...)`, `push_mode` = `Auto`): Uses `solana_transaction` /
///   `solana_message` API types through `sign_and_send_transaction`. Fordefi will modify the
///   transaction (at minimum updating the blockhash, and optionally adding priority fees),
///   **auto-broadcasts** it on-chain (`push_mode: "auto"`), and returns the signature, the
///   on-chain identifier. `sign_transaction` is rejected in this mode.
/// - **Native manual** (`chain` = `Some(...)`, `push_mode` = `Manual`): for the
///   unsigned requests supported here, Fordefi may replace the blockhash and
///   manage priority-fee instructions without broadcasting. Fordefi does not
///   broadcast, so this mode signs through `sign_transaction`, which replaces the
///   caller's transaction with Fordefi's — only after all other content is
///   validated — and returns a non-empty serialized transaction.
///   `sign_and_send_transaction` is rejected in this mode.
...
impl SolanaSigner for FordefiSigner {
    ...
    async fn sign_transaction(
        ...
        // alongside black box mode.
        let signed_transaction = if self.chain.is_some() {
            self.sign_and_serialize_native_manual(tx).await?
        } else {
            self.sign_and_serialize_black_box(tx).await?
        };
    ...
    async fn sign_and_send_transaction(
        ...
        if self.push_mode != FordefiPushMode::Auto {
            return Err(SignerError::SigningFailed(
                "Fordefi native manual mode does not broadcast; sign the transaction and \
                 broadcast the result"
                    .to_string(),
            ));
        }
        let (_, signature) = self.sign_and_serialize_native_auto(tx).await?;
        Ok(signature)
    }
