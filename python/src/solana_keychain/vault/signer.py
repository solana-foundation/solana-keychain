...
class VaultSigner(TransactionSigner):
    ...
    def _latest_key_is_configured_pubkey(self, data: dict[str, Any]) -> bool:
        ...
        keys = data.get("keys")
        version = keys.get(str(data.get("latest_version"))) if isinstance(keys, dict) else None
        encoded = version.get("public_key", "") if isinstance(version, dict) else ""
        try:
            return Pubkey(base64.b64decode(str(encoded), validate=True)) == self._pubkey
