...
/**
 * Golden vector: 32 distinct bytes and their base58 address. Deliberately not
 * an all-zero key, whose address is all-`1`s under any slice offset and so
 * cannot catch an off-by-one in the SubjectPublicKeyInfo prefix length. The
 * leading `0x00` still exercises base58 leading-zero handling.
 */
const GOLDEN_KEY = new Uint8Array(Array.from({ length: 32 }, (_, i) => i));
const GOLDEN_ADDRESS = '1thX6LZfHDZZKUs92febYZhYRcXddmzfzF2NvTkPNE';
...
describe('addressFromEd25519Key', () => {
    it('renders a 32-byte key as a base58 address', () => {
        expect(addressFromEd25519Key(GOLDEN_KEY)).toBe(GOLDEN_ADDRESS);
    });
...
describe('addressFromSpkiDer', () => {
    it('extracts the address from an Ed25519 SubjectPublicKeyInfo', () => {
        expect(addressFromSpkiDer(spkiDer(GOLDEN_KEY))).toBe(GOLDEN_ADDRESS);
    });

    it('rejects DER that is not an Ed25519 SubjectPublicKeyInfo', () => {
        const foreignOid = spkiDer(GOLDEN_KEY);
        foreignOid[8] = 0x71;
...
describe('addressFromSpkiPem', () => {
    it('extracts the address from a PEM-wrapped SubjectPublicKeyInfo', () => {
        const body = getBase64Decoder().decode(spkiDer(GOLDEN_KEY));
        const pem = `-----BEGIN PUBLIC KEY-----\n${body}\n-----END PUBLIC KEY-----\n`;
        expect(addressFromSpkiPem(pem)).toBe(GOLDEN_ADDRESS);
    });
