package dfns

import (
	"crypto/ed25519"
	"crypto/x509"
	"encoding/pem"
	"testing"

	"github.com/solana-foundation/solana-keychain/go/core"
)

// PEM fixtures ported from the Rust dfns::auth tests.

// testEd25519PEM is an Ed25519 test key in PKCS#8 PEM format.
const testEd25519PEM = "-----BEGIN PRIVATE KEY-----\n" +
	"MC4CAQAwBQYDK2VwBCIEIJ+DYvh6SEqVTm50DFtMDoQikUmifl1yiWd+IiYyoHBD\n" +
	"-----END PRIVATE KEY-----"

// testP256PEM is a P256/ECDSA test key in PKCS#8 PEM format.
const testP256PEM = "-----BEGIN PRIVATE KEY-----\n" +
	"MIGHAgEAMBMGByqGSM49AgEGCCqGSM49AwEHBG0wawIBAQQgNVGLQN9VkU26M2JG\n" +
	"3hbSFACbGLXkQlB69ZxAhXGqf/mhRANCAATjr6H28PJiFSlRz9kfkzu9Fy6vt1uY\n" +
	"9Egu4yP/e2qnDZ+SjpcQo1hpF6Cb1h6S1a2b7qi3IEEnh+d/vzlOHAaf\n" +
	"-----END PRIVATE KEY-----"

// testP256SEC1PEM is a P256/ECDSA test key in SEC1 PEM format.
const testP256SEC1PEM = "-----BEGIN EC PRIVATE KEY-----\n" +
	"MHcCAQEEIGa93+PpxzDlIywW+Al/cpIAGzLKwGwIDWpgwrJ+ht9ZoAoGCCqGSM49\n" +
	"AwEHoUQDQgAE0Mi+Kw78tyVMPAGb6a6Nwn/yiz65gKVBS+nT171vqgLzoHwf51iU\n" +
	"TLWfftn3ZyCvKLzTN5pd1Up982TKelcbFw==\n" +
	"-----END EC PRIVATE KEY-----"

// testRSAPEM is an RSA-2048 test key in PKCS#8 PEM format.
const testRSAPEM = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`

// testEd25519CredentialPublicKey extracts the Ed25519 public key from
// testEd25519PEM so tests can verify user-action challenge signatures.
func testEd25519CredentialPublicKey(t *testing.T) ed25519.PublicKey {
	t.Helper()
	block, _ := pem.Decode([]byte(testEd25519PEM))
	if block == nil {
		t.Fatal("failed to decode test PEM")
	}
	key, err := x509.ParsePKCS8PrivateKey(block.Bytes)
	if err != nil {
		t.Fatal(err)
	}
	return key.(ed25519.PrivateKey).Public().(ed25519.PublicKey)
}

func TestSignChallengeEd25519(t *testing.T) {
	data := []byte("test challenge data")
	sig, err := signChallenge(testEd25519PEM, data)
	if err != nil {
		t.Fatal(err)
	}
	if len(sig) != 64 {
		t.Errorf("signature length = %d, want 64", len(sig))
	}
	if !ed25519.Verify(testEd25519CredentialPublicKey(t), data, sig) {
		t.Error("Ed25519 challenge signature should verify")
	}
}

func TestSignChallengeP256(t *testing.T) {
	cases := map[string]string{
		"pkcs8": testP256PEM,
		"sec1":  testP256SEC1PEM,
	}
	for name, pemKey := range cases {
		t.Run(name, func(t *testing.T) {
			sig, err := signChallenge(pemKey, []byte("test challenge data"))
			if err != nil {
				t.Fatal(err)
			}
			// DER-encoded ECDSA signatures (typically 70-72 bytes).
			if len(sig) < 68 || len(sig) > 72 {
				t.Errorf("DER signature length = %d, want 68..72", len(sig))
			}
			if sig[0] != 0x30 {
				t.Errorf("signature[0] = %#x, want DER SEQUENCE tag 0x30", sig[0])
			}
		})
	}
}

func TestSignChallengeRSA(t *testing.T) {
	sig, err := signChallenge(testRSAPEM, []byte("test challenge data"))
	if err != nil {
		t.Fatal(err)
	}
	if len(sig) != 256 {
		t.Errorf("signature length = %d, want 256", len(sig))
	}
}

func TestSignChallengeInvalidKey(t *testing.T) {
	_, err := signChallenge("not-a-pem-key", []byte("test"))
	if err == nil {
		t.Fatal("expected error for invalid PEM key")
	}
	if code, _ := core.CodeOf(err); code != core.CodeInvalidPrivateKey {
		t.Errorf("got %s, want INVALID_PRIVATE_KEY", code)
	}
}
