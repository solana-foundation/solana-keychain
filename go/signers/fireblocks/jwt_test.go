package fireblocks

import (
	"crypto/sha256"
	"encoding/base64"
	"encoding/hex"
	"encoding/json"
	"strings"
	"testing"

	"github.com/solana-foundation/solana-keychain/go/core/v2"
)

// testRSAKey is a throwaway PKCS#8 RSA key for unit tests only.
const testRSAKey = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`

// decodeJWTPayload splits a compact JWT and JSON-decodes its payload segment.
func decodeJWTPayload(t *testing.T, token string) map[string]any {
	t.Helper()
	parts := strings.Split(token, ".")
	if len(parts) != 3 {
		t.Fatalf("jwt has %d parts, want 3", len(parts))
	}
	payloadBytes, err := base64.RawURLEncoding.DecodeString(parts[1])
	if err != nil {
		t.Fatalf("payload should be base64url: %v", err)
	}
	var payload map[string]any
	if err := json.Unmarshal(payloadBytes, &payload); err != nil {
		t.Fatalf("payload should be valid json: %v", err)
	}
	return payload
}

func TestCreateJWT(t *testing.T) {
	key, err := parseSigningKey(testRSAKey)
	if err != nil {
		t.Fatalf("failed to parse test RSA key: %v", err)
	}

	body := `{"test": "body"}`
	token, err := createJWT("test-api-key", key, "/v1/transactions", body)
	if err != nil {
		t.Fatal(err)
	}

	payload := decodeJWTPayload(t, token)
	iat, ok := payload["iat"].(float64)
	if !ok {
		t.Fatal("iat should be present")
	}
	nbf, ok := payload["nbf"].(float64)
	if !ok {
		t.Fatal("nbf should be present")
	}
	exp, ok := payload["exp"].(float64)
	if !ok {
		t.Fatal("exp should be present")
	}
	if iat != nbf {
		t.Errorf("iat = %v, want equal to nbf = %v", iat, nbf)
	}
	if got, want := int64(exp-iat), int64(jwtTTLSecs+jwtSkewLeewaySecs); got != want {
		t.Errorf("exp - iat = %d, want %d", got, want)
	}

	if got := payload["sub"]; got != "test-api-key" {
		t.Errorf("sub = %v, want test-api-key", got)
	}
	if got := payload["uri"]; got != "/v1/transactions" {
		t.Errorf("uri = %v, want /v1/transactions", got)
	}
	wantHash := sha256.Sum256([]byte(body))
	if got := payload["bodyHash"]; got != hex.EncodeToString(wantHash[:]) {
		t.Errorf("bodyHash = %v, want sha256 hex of body", got)
	}
	if nonce, _ := payload["nonce"].(string); len(nonce) != 36 {
		t.Errorf("nonce = %q, want UUIDv4-shaped string", payload["nonce"])
	}
}

func TestParseSigningKeyInvalid(t *testing.T) {
	_, err := parseSigningKey("invalid-key")
	if err == nil {
		t.Fatal("expected error for invalid RSA key")
	}
	if code, _ := core.CodeOf(err); code != core.CodeInvalidPrivateKey {
		t.Errorf("got %s, want INVALID_PRIVATE_KEY", code)
	}
}
