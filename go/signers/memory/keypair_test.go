...
func TestPrivateKeyFromBytesRejectsMismatchedPublicHalf(t *testing.T) {
	key := make([]byte, 64)
	copy(key, testutils.TestPrivateKey())
	key[63] ^= 0xFF // corrupt the embedded public key
	_, err := privateKeyFromBytes(key)
	if err == nil {
		t.Fatal("expected error for mismatched public half")
	}
	if code, _ := core.CodeOf(err); code != core.CodeInvalidPrivateKey {
		t.Errorf("got %s, want INVALID_PRIVATE_KEY", code)
	}
