...
- **Broadcast-managed signers** (Crossmint, Fordefi native auto mode) never mutate the caller's transaction, and a failure does not mean nothing landed. `BROADCAST_UNCONFIRMED` carries a provider transaction id only when the create was accepted, and always carries the message-derived idempotency key the create went out under: without an id, recovery rests on that key, which makes a byte-identical resend safe and a rebuilt transaction a new transfer. Fireblocks PROGRAM_CALL sends the same kind of key as `externalTxId`; its RAW path sends none, since the same message may legitimately be signed twice.
...
- **CDP** accepts UTF-8 message payloads only, and needs its `network` config set to sign a transaction carrying address lookup tables: CDP resolves the tables server-side, so signing one without it fails locally.
