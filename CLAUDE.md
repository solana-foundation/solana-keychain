...
- **Compute Budget program facts:** declared locally in `packages/fordefi/src/compute-budget.ts`, not pulled from `@solana-program/compute-budget` — that client peer-requires `@solana/kit` and Node >= 24, which would raise the package's floor (mirrors the Rust-side decision). Pinned to canonical encodings in `compute-budget.test.ts`.
