...
- **Fordefi push mode:** `FordefiSignerConfig.push_mode` selects native `Auto` (Fordefi broadcasts) or `Manual` (Fordefi rewrites and signs, caller broadcasts); `None` means `Auto`, and `Manual` requires `chain`. `broadcasts_transactions()` is true only for `Auto`, which is what `sign_and_send` keys on.
- **Compute Budget program ID:** declared locally in each `sdk_adapter/v*.rs` rather than pulled from `solana-compute-budget-interface`, so non-Fordefi consumers do not carry the dependency. `sdk_adapter::tests` pins it to its canonical bytes.
...
- **Contract** (`solana_keychain.core`): `SolanaSigner` ABC — `pubkey` property, async `sign_transaction()` / `sign_message()` / `is_available()`. `sign_transaction` takes a `VersionedTransaction` (legacy, v0 or v1) and returns `SignedTransaction` with `is_complete` and `transaction`. `transaction` is always the authoritative signed object: most backends sign in place and return the same object, but a provider may return a rewritten message (Fordefi native manual mode), which `solders` cannot apply in place — so always continue from `result.transaction`, never the object you passed in. `classify_signed_transaction()` is the single chokepoint that populates it. Use `signed_message_bytes()` for the bytes a signature covers; never hand-roll the version prefix.
